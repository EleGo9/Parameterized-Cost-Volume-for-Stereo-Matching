...
from core.update import BasicMultiUpdateBlock, DisparityDecoder
from core.extractor import MultiBasicEncoder, ResidualBlock, HDAEncoder
...
class StereoHDA(nn.Module):
    def __init__(self, args):
        ...
        # print('cnet', len(self.cnet))
        # print('cnet[0]', self.cnet[0].shape)
        ...
        self.hda_extractor = HDAEncoder(channels=args.extractor_feat)
        self.decoder = DisparityDecoder(max_disp, channels=args.extractor_feat)
    ...
    def forward(self, image1, image2, iters=12, test_mode=False, init_param=None, cascade=False):
        ...
        x1_l, x2_l, x3_l, x4_l, x5_l = self.hda_extractor(image1)
        x1_r, x2_r, x3_r, x4_r, x5_r = self.hda_extractor(image2)

        if DEBUG:
            print(x1_l.shape, x2_l.shape, x3_l.shape, x4_l.shape, x5_l.shape)
        stereo_disp, disp2, disp3, disp4, disp5 = self.decoder([(x1_l, x2_l, x3_l, x4_l, x5_l), (x1_r, x2_r, x3_r, x4_r, x5_r )])
        # disp = self.dummy_model(torch.cat((image1, image2), dim=0))
