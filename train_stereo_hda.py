...
def dummy_loss(disp_pred, disp_gt, max_disp=512):
    # print(disp_pred[0].shape, disp_gt.shape)
    # Try epe_final = torch.abs(disp_refine - disp_gt)
    #    epe_final = epe_final.view(-1)[valid.view(-1)]
    # instead of your epe computation.
    stereo_disp, disp2, disp3, disp4, disp5 = disp_pred
    
    epe_final_loss = torch.mean(torch.abs(stereo_disp - disp_gt))
    epe_final = torch.abs(stereo_disp - disp_gt)
    metrics = {
    ...
    return epe_final_loss, metrics

def hda_loss(disp_pred, disp_gt, max_disp=512):
    # print(disp_pred[0].shape, disp_gt.shape)
    # Try epe_final = torch.abs(disp_refine - disp_gt)
    #    epe_final = epe_final.view(-1)[valid.view(-1)]
    # instead of your epe computation.
    stereo_disp, disp2, disp3, disp4, disp5 = disp_pred
    valid = (disp_gt < max_disp) & (disp_gt >= 0)
    assert valid.shape == disp_gt.shape, [valid.shape, disp_gt.shape]
    assert not torch.isinf(disp_gt[valid.bool()]).any()
    disp_gt2 = F.interpolate(disp_gt, scale_factor=0.5, mode='bilinear')
    disp_gt3 = F.interpolate(disp_gt2, scale_factor=0.5, mode='bilinear')
    disp_gt4 = F.interpolate(disp_gt3, scale_factor=0.5, mode='bilinear')
    disp_gt5 = F.interpolate(disp_gt4, scale_factor=0.5, mode='bilinear')
    gamma = 0.100
    epe_2 = F.smooth_l1_loss(disp2, disp_gt2, size_average=True) 
    epe_3 = F.smooth_l1_loss(disp3, disp_gt3, size_average=True)
    epe_4 = F.smooth_l1_loss(disp4, disp_gt4, size_average=True)
    epe_5 = F.smooth_l1_loss(disp5, disp_gt5, size_average=True)
    if (valid == False).all():
        disp_loss = 0
        metrics = {
            'epe': 0.,
        ...
        epe_final_loss = F.smooth_l1_loss(stereo_disp, disp_gt, size_average=True) + gamma*((epe_2+epe_3+epe_4+epe_5))
    # epe_final_loss = torch.mean(torch.abs(stereo_disp - disp_gt)) #+ gamma*(epe_2+epe_3+epe_4+epe_5)
    epe_final = torch.abs(stereo_disp - disp_gt)
    metrics = {
            # 'epe': epe.mean().item(),
...
def fetch_optimizer(args, model):
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(optimizer, args.lr, args.num_steps + 100,
                                              pct_start=args.pct_start, cycle_momentum=False, anneal_strategy=args.anneal_st)
    # scheduler = optim.lr_scheduler.CyclicLR(optimizer, base_lr=args.base_lr, max_lr=args.lr,step_size_up=8, step_size_down=10000, mode="triangular2") #,gamma=0.90)
    ...
    while should_keep_training:
        for i_batch, (files_path, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            if args.cascade:
                ...
                loss, metrics = hda_loss(output_list, disp, args.max_disp)
                # loss, metrics = sequence_loss(*output_list, disp, valid, gauss_num_train, args.max_disp)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='hda_stereo', help="name your experiment")

    # Training parameters
    parser.add_argument('--restore_ckpt', help="restore checkpoint")
    parser.add_argument('--mixed_precision', action='store_true', help='use mixed precision')
    parser.add_argument('--batch_size', type=int, default=8, help="batch size used during training")
    parser.add_argument('--train_datasets', nargs='+', default=['sceneflow', 'middlebury_F', 'kitti', 'booster', 'crestereo', 'cemstereo'], help="training dataset(s)")
    parser.add_argument('--lr', type=float, default=0.0002, help="max learning rate")
    parser.add_argument('--anneal_st', default='linear', help="anneal_strategy 'cos' or 'linear'")
    ...
    parser.add_argument('--log_freq', type=int, default=200, help="the frequency of logging")
    ...
    parser.add_argument('--lr_slope', type=int, help="number of iter for the learning rate to decay. Bigger number means slower decay", default=10100)
    parser.add_argument('--base_lr', type=float, help="number of minimum lr", default=0.0000001)
    ...
    parser.add_argument('--extractor_feat', help='list of output channels from the highest to the lowest resolution', default=[16, 64, 64, 128, 128])
